...
{
    public class GetVacanciesByDescriptionHandler : IRequestHandler<GetVacanciesByDescriptionRequest, List<VacancyWithScoreResponseModel>>
    {
        private readonly CoursworkContext _context;
        private readonly IPartialSearchService _partialSearchService;

        public GetVacanciesByDescriptionHandler(CoursworkContext context, IPartialSearchService partialSearchService)
        {
            _context = context;
            _partialSearchService = partialSearchService;
        }

        public async ValueTask<List<VacancyWithScoreResponseModel>> Handle(GetVacanciesByDescriptionRequest request, CancellationToken cancellationToken)
        {
            var vacancies = await (
            ...
            {
                var searchList = vacancy.Description.Split(' ');
                var searchModel = new PartialSearchModel
                {
                    SearchList = new List<string>(searchList),
                    Requests = new List<string>(request.Description)
                };
