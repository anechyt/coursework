...
{
    public class GetCandidatesByResumeHandler : IRequestHandler<GetCandidatesByResumeRequest, List<CandidateWithScoreResponseModel>>
    {
        private readonly CoursworkContext _context;
        private readonly IPartialSearchService _partialSearchService;

        public GetCandidatesByResumeHandler(CoursworkContext context,
        ...
        public async ValueTask<List<CandidateWithScoreResponseModel>> Handle(GetCandidatesByResumeRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                var searchList = candidate.Resume.Split(' ');
                var searchModel = new PartialSearchModel
                {
                    SearchList = new List<string>(searchList),
                    Requests = new List<string>(request.Resume)
                };
